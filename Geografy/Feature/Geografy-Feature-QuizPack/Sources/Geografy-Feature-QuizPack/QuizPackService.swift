...
public final class QuizPackService {
    ...
    public func progress(for levelID: String) -> QuizPackProgress? {
        progressByLevel[levelID]
    }

    public func stars(for levelID: String) -> Int {
        progressByLevel[levelID]?.stars ?? 0
    }

    public func completedLevelCount(for pack: QuizPack) -> Int {
        pack.levels.filter { progressByLevel[$0.id] != nil }.count
    }

    public func totalStars(for pack: QuizPack) -> Int {
        pack.levels.reduce(0) { $0 + stars(for: $1.id) }
    }

    public func maxStars(for pack: QuizPack) -> Int {
        pack.levels.count * 3
    }

    public func isPackCompleted(_ pack: QuizPack) -> Bool {
        completedLevelCount(for: pack) == pack.levelCount
    }

    public func isPackUnlocked(
        _ pack: QuizPack,
        allPacks: [QuizPack]
    ) -> Bool {
...
extension QuizPackService {
    public static func makeAllPacks(
        countries: [Country]
    ) -> [QuizPack] {
        var packs: [QuizPack] = []
