...
@Observable
final class CountryNicknamesService {
    private(set) var nicknames: [CountryNickname] = CountryNicknamesService.allNicknames

    func nicknames(for category: NicknameCategory?) -> [CountryNickname] {
        guard let category else { return nicknames }
        return nicknames.filter { $0.category == category }
    }

    func filteredNicknames(query: String, category: NicknameCategory?) -> [CountryNickname] {
