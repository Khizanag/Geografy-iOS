...
final class FlashcardService {
    ...
    func recordReview(cardID: String, result: FlashcardReviewResult) {
        let existing = spacedRepetitionStore[cardID] ?? .makeNew()
        spacedRepetitionStore[cardID] = existing.updated(with: result)
        saveData()
    }

    func spacedRepetitionData(for cardID: String) -> SpacedRepetitionData {
        spacedRepetitionStore[cardID] ?? .makeNew()
    }

    func dueCards(from items: [FlashcardItem]) -> [FlashcardItem] {
        items.filter { spacedRepetitionData(for: $0.id).isDueForReview }
    }

    func masteryPercentage(for items: [FlashcardItem]) -> Double {
