...
@Observable
@MainActor
public final class ExploreGameService {
    private(set) var statistics = Statistics()

    private let countryDataService: CountryDataService

    public init(countryDataService: CountryDataService) {
...
// MARK: - Game Creation
extension ExploreGameService {
    public func makeDailyGame() -> ExploreGameState? {
    ...
    public func makePracticeGame() -> ExploreGameState? {
        let countries = countryDataService.countries
        guard !countries.isEmpty else { return nil }
...
extension ExploreGameService {
    public func recordResult(_ result: ExploreGameResult) {
